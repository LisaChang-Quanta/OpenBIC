...
#define MP2891_DEV_ID 0x2891 // TODO:??
...
static uint8_t mp2891_check_err_status(uint8_t bus, uint8_t addr, uint8_t page)
{
	if (mp2891_set_page(bus, addr, page) == false) {
...
static bool mp2891_store(uint8_t bus, uint8_t addr, uint8_t page)
{
	if (mp2891_set_page(bus, addr, page) == false) {
	...
	k_msleep(200); // TODO:50ms+2ms*modified_reg_number

	k_msleep(100); // TODO:Toggling VDD33 power supply Wait 100ms

	if (mp2891_check_err_status(bus, addr, page)) {
	...
	for (line_idx = 0; line_idx < dev_cfg.wr_cnt; line_idx++) {
		...
		if ((cur_data->page == VR_MPS_PAGE_0) || (cur_data->page == VR_MPS_PAGE_1)) {
			if (cur_data->page != last_page) {
				if (line_idx != 0) {
					if (mp2891_store(bus, addr, VR_MPS_PAGE_0) == false) {
						LOG_ERR("Failed to store USER data to MTP at page %d!",
							last_page);
						goto exit;
					}
					if (mp2891_store(bus, addr, VR_MPS_PAGE_1) == false) {
						LOG_ERR("Failed to store USER data to MTP at page %d!",
							last_page);
						goto exit;
					}
